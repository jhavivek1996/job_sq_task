...
module.exports = {
  create: async (data, callBack) => {
    try {
      await pool.query(
        `insert into registration(firstName, lastName, gender, email, password, number,type) values(?,?,?,?,?,?,?)`,
        [
          data.firstName,
          data.lastName,
          data.gender,
  ...
  show: async (data, callBack) => {
    try {
      await pool.query(
        "select id,firstName,gender,email,number from registration",
        (error, results, fields) => {
          if (error) {
            return callBack(error);
          }
  ...
  updates: async (data, callBack) => {
    try {
      await pool.query(
        ...
        [data.first_name, data.last_name, data.gender, data.number, data.id],
  ...
  adminLogin: async (email, callBack) => {
    try {
      await pool.query(
        `select * from registration where email = ? where type='admin'`,
        [email],
        (error, results, fields) => {
          if (error) {
  ...
  CandidateLogin: async (email, callBack) => {
    try {
      await pool.query(
        `select * from registration where email = ? where type='candidate'`,
        [email],
        (error, results, fields) => {
          if (error) {
            callBack("Error occured");
  ...
  RecruiterLogin: async (email, callBack) => {
    try {
      await pool.query(
        `select * from registration where email = ? where type='recruiter'`,
        [email],
        (error, results, fields) => {
          if (error) {
            callBack("Error occured");
